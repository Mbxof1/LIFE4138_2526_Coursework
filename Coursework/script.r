...
A_vs_Bvol <- ggplot(data=A_vs_B, aes(x=log2FoldChange, y=-log10(padj))) + geom_point()
A_vs_Fvol <- ggplot(data=A_vs_F, aes(x=log2FoldChange, y=-log10(padj))) + geom_point()
...
A_vs_BMAplot <- ggplot(data=A_vs_B,aes(x=log2(baseMean), y=log2FoldChange, colour=threshold)) + geom_point() 
A_vs_FMAplot <- ggplot(data=A_vs_F,aes(x=log2(baseMean), y=log2FoldChange, colour=threshold)) + geom_point() 
plot(A_vs_BMAplot)
plot(A_vs_FMAplot)


#Plot histograms for both datasets, demonstrating the distribution of adjusted p values. Number of bins set to 50.
A_vs_Bhist <- ggplot(A_vs_B,aes(x=padj)) + geom_histogram(binwidth=.02)
A_vs_Fhist <- ggplot(A_vs_F,aes(x=padj)) + geom_histogram(binwidth=.02)
plot(A_vs_Bhist)
plot(A_vs_Fhist)
...
Combdf %>% ggplot(aes(x= dataset, y = gene_id, fill=padj)) + geom_tile()
Combdf %>% ggplot(aes(x= dataset, y = gene_id, fill=log2FoldChange)) + geom_tile()


#Combine the previous significant gene data frames, 
Combsigdf <- bind_rows(A_vs_B_Sig=A_vs_B, A_vs_F_Sig=A_vs_F,.id="dataset")
View(Combsigdf)
Combsigdfhead %>% ggplot(aes(x= dataset, y = gene_id, fill=padj)) + geom_tile()
Combsigdfhead %>% ggplot(aes(x= dataset, y = gene_id, fill=log2FoldChange)) + geom_tile()
...
#Print a table of genes which demonstrate significantly different expression from baseline
View(A_vs_B_Sig)
View(A_vs_F_Sig)
